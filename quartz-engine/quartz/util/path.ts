...
export function transformLink(src: FullSlug, target: string, opts: TransformOptions): RelativeURL {
  // Map any internal link target pointing to the Obsidian note name 'Home' (or Home#anchor)
  // to 'index' so it resolves directly to the root homepage instead of the broken '/Home' path.
  const [targetPath, targetAnchor] = splitAnchor(decodeURI(target))
  const isHomeLink = targetPath.toLowerCase() === "home" || targetPath.toLowerCase() === "./home" || targetPath.toLowerCase() === "/home"
  const cleanTarget = isHomeLink ? `index${targetAnchor}` : target
  let targetSlug = transformInternalLink(cleanTarget)
